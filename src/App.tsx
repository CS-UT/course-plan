...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] dark:bg-[#1C1816] text-[#3D2B1F] dark:text-[#E8DED2] transition-colors">
      {/* Header */}
      <header className="bg-[#FFF8F0] dark:bg-[#2A2420] border-b border-[#E8DED2] dark:border-[#3D352E] sticky top-0 z-40 transition-colors shadow-sm shadow-amber-900/5">
        <div className="max-w-[1600px] mx-auto px-4 py-2.5 flex items-center justify-between gap-2">
          <span className="text-sm text-[#8B7355] dark:text-[#9C8B7A] hidden sm:inline truncate">
            {data.semesterLabel} — {data.department}
          </span>
          <div className="flex items-center gap-2 sm:gap-3 mr-auto sm:mr-0">
            <span className="text-sm font-semibold bg-primary-100 dark:bg-primary-900/40 text-primary-800 dark:text-primary-200 px-3 py-1 rounded-full whitespace-nowrap">
            ...
            <a
              ...
              className="p-2 rounded-xl hover:bg-[#F0E6D8] dark:hover:bg-[#3D352E] transition-colors text-[#8B7355] dark:text-[#9C8B7A]"
              title="GitHub"
            >
            ...
            <button
              onClick={toggleDark}
              className="p-2 rounded-xl hover:bg-[#F0E6D8] dark:hover:bg-[#3D352E] transition-colors cursor-pointer text-[#8B7355] dark:text-[#9C8B7A]"
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="mt-4">
            <button
              ...
              className="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 font-semibold transition-colors cursor-pointer"
            >
  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        className="fixed bottom-5 right-4 bg-primary-600 hover:bg-primary-700 text-white px-5 py-3.5 rounded-full shadow-lg shadow-amber-900/20 z-50 font-semibold cursor-pointer text-sm transition-colors"
      >
        + افزودن درس
      </button>
      {open && (
        <div className="fixed inset-0 z-50 bg-black/30" onClick={() => setOpen(false)}>
          <div
            className="absolute bottom-0 left-0 right-0 bg-[#FFF8F0] dark:bg-[#2A2420] rounded-t-2xl max-h-[85vh] overflow-y-auto pb-6"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Drag handle */}
            <div className="sticky top-0 bg-[#FFF8F0] dark:bg-[#2A2420] pt-3 pb-2 px-4 z-10 rounded-t-2xl">
              <div className="w-10 h-1 bg-[#E8DED2] dark:bg-[#3D352E] rounded-full mx-auto mb-3" />
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-lg">انتخاب درس</h3>
                <button onClick={() => setOpen(false)} className="text-[#8B7355] dark:text-[#9C8B7A] p-1.5 -m-1.5 rounded-xl hover:bg-[#F0E6D8] dark:hover:bg-[#3D352E] cursor-pointer">
