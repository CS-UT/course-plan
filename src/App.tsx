...
function MobileBanner() {
  ...
  return (
    <div className="lg:hidden bg-[#0d1a2e] border-b border-[#1a2a4e] px-4 py-2.5 flex items-center justify-between gap-3">
      ...
      <button
        ...
        className="text-[#00f5ff] text-xs shrink-0 cursor-pointer font-medium"
      >
        متوجه شدم
      </button>
    </div>
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] dark:bg-[#06060b] text-[#e0e0e8] transition-colors">
      {/* Header */}
      <header className="bg-[#0d0d18]/90 backdrop-blur-md border-b border-[#1a1a2e] sticky top-0 z-40 transition-colors">
        <div className="max-w-[1600px] mx-auto px-4 py-2.5 flex items-center justify-between gap-2">
          <span className="text-sm text-[#4a4a6a] hidden sm:inline truncate">
            {data.semesterLabel} — {data.department}
          </span>
          <div className="flex items-center gap-2 sm:gap-3 mr-auto sm:mr-0">
            <span className="text-sm font-medium bg-[#00f5ff]/10 text-[#00f5ff] px-3 py-1 rounded-full whitespace-nowrap border border-[#00f5ff]/20 shadow-[0_0_10px_rgba(0,245,255,0.1)]">
            ...
            <a
              ...
              className="p-2 rounded-lg hover:bg-[#1a1a2e] transition-colors text-[#6a6a8a] hover:text-[#00f5ff]"
              title="GitHub"
            >
            ...
            <button
              onClick={toggleDark}
              className="p-2 rounded-lg hover:bg-[#1a1a2e] transition-colors cursor-pointer text-[#6a6a8a] hover:text-[#ff00ff]"
              title={dark ? 'حالت میدنایت' : 'حالت توایلایت'}
            >
              {dark ? (
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="mt-4">
            <button
              ...
              className="text-sm text-[#00f5ff] hover:text-[#00ff88] font-medium transition-colors cursor-pointer"
            >
  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        className="fixed bottom-5 right-4 bg-[#00f5ff] text-[#0a0a0f] px-5 py-3.5 rounded-full shadow-[0_0_20px_rgba(0,245,255,0.3)] z-50 font-bold cursor-pointer text-sm"
      >
        + افزودن درس
      </button>
      {open && (
        <div className="fixed inset-0 z-50 bg-black/60" onClick={() => setOpen(false)}>
          <div
            className="absolute bottom-0 left-0 right-0 bg-[#0d0d18] border-t border-[#1a1a2e] rounded-t-2xl max-h-[85vh] overflow-y-auto pb-6"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Drag handle */}
            <div className="sticky top-0 bg-[#0d0d18] pt-3 pb-2 px-4 z-10 rounded-t-2xl">
              <div className="w-10 h-1 bg-[#2a2a4e] rounded-full mx-auto mb-3" />
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-lg text-[#e0e0e8]">انتخاب درس</h3>
                <button onClick={() => setOpen(false)} className="text-[#4a4a6a] p-1.5 -m-1.5 rounded-lg hover:bg-[#1a1a2e] cursor-pointer">
