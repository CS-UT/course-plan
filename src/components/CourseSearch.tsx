  ...
  return (
    <div className="flex flex-col gap-3">
      <input
        ...
        className="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent bg-white"
      />
    ...
    <div
      className={`p-3 rounded-xl border text-sm transition-all cursor-pointer ${
        selected
          ...
          : hasConflict
            ? 'bg-white border-gray-200 hover:bg-warning-50 hover:border-warning-300'
    ...
    >
      <div className="flex justify-between items-start gap-2">
        <div className="flex-1 min-w-0">
          <div className="font-bold text-gray-900 truncate">{course.courseName}</div>
          <div className="text-gray-600 text-xs mt-1">{course.professor}</div>
        </div>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5 text-xs text-gray-500">
        ...
        {genderLabel && (
          <span className="bg-gray-100 px-1.5 py-0.5 rounded">{genderLabel}</span>
        )}
      </div>
      ...
      {hasConflict && !selected && (
        <div className="mt-2 text-xs text-warning-600 bg-warning-50 rounded px-2 py-1">
