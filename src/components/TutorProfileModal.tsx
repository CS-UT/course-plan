                ...
                return (
                  <span
                    ...
                    className={`text-sm px-2.5 py-1 rounded-lg ${
                      val
                        ? 'bg-green-500/15 dark:bg-green-400/10 text-green-700 dark:text-green-400'
        ...
        <div className="flex flex-col gap-2.5">
          {Object.entries(review.sections!).map(([key, content]) => (
            <div key={key}>
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400 bg-white/20 dark:bg-white/5 rounded-xl backdrop-blur-sm px-3 py-2.5 leading-relaxed whitespace-pre-line">
                {content}
              </div>
            </div>
          ))}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1.5">
            {review.comments.map((c, i) => (
              <div
                key={i}
                className="text-sm text-gray-600 dark:text-gray-400 bg-white/20 dark:bg-white/5 rounded-xl backdrop-blur-sm px-3 py-2.5 leading-relaxed whitespace-pre-line"
              >
                {c}
              </div>
            ))}
            ...
            {tutor.reviews.map((r, i) => (
              <button
                ...
                className={`text-sm px-3 py-1.5 rounded-lg cursor-pointer transition-colors ${
                  i === activeReviewIdx
                    ? 'backdrop-blur-sm bg-primary-500/15 dark:bg-primary-400/10 text-primary-700 dark:text-primary-300 font-medium'
                    : 'bg-white/20 dark:bg-white/5 text-gray-600 dark:text-gray-400 hover:bg-white/35 dark:hover:bg-white/10'
                }`}
              >
